...
def validateConfig(configDict):
    ...
    smilesColumn = configDict["smilesColumn"]
    requiredInputColumns = configDict["requiredInputColumns"]
    if smilesColumn not in requiredInputColumns:
        raise ValueError(
            f"smilesColumn '{smilesColumn}' must be listed in requiredInputColumns. "
            f"Currently: {requiredInputColumns}"
        )


def buildPaths(configDict):
    dataDir = os.path.expandvars(configDict["dataDir"])
    artResultsSubdir = os.path.expandvars(configDict["artResultsSubdir"])
...
def deduplicateForPrediction(inputDF, smilesColumn):
    return (
        inputDF[[smilesColumn]]
        .dropna()
        .drop_duplicates()
...
def applyDiversityFilter(
    ...
    Works with any SMILES column name passed via smilesColumn.
    """
    ...
    for startIdx in range(0, totalUniqueInput, diversityChunkSize):
        ...
        for smi in chunkDF[smilesColumn]:
            mol = Chem.MolFromSmiles(smi)
            if mol is None:
                continue
    ...
    if useCanonicalFiltering:
        ...
        mask = inputDF[smilesColumn].isin(selectedSmilesSet)
        filteredInputDF = inputDF.loc[mask].reset_index(drop=True)
...
def safePredictBatch(model, smilesBatch, smilesColumn):
    ...
    batchDF = pd.DataFrame({smilesColumn: smilesBatch}).reset_index(drop=True)
    batchDF = pd.concat([batchDF, predictionsDF], axis=1)
    return batchDF


def loadCheckpoint(checkpointCsvPath, smilesColumn):
    if os.path.exists(checkpointCsvPath):
        checkpointDF = pd.read_csv(checkpointCsvPath)
        if smilesColumn not in checkpointDF.columns:
            raise ValueError(
                f"Checkpoint exists but does not contain the expected SMILES column '{smilesColumn}'."
            )
        checkpointDF = checkpointDF.drop_duplicates(subset=smilesColumn, keep="last").reset_index(drop=True)
        return checkpointDF
    return None
...
def compactCheckpoint(checkpointCsvPath, smilesColumn):
    checkpointDF = pd.read_csv(checkpointCsvPath)
    checkpointDF = checkpointDF.drop_duplicates(subset=smilesColumn, keep="last").reset_index(drop=True)
...
def mergePredictionsBack(inputDF, predictionsDF, smilesColumn):
    return inputDF.merge(predictionsDF, on=smilesColumn, how="left")
...
def main():
    ...
    print(f"smilesColumn            : {smilesColumn}")
    ...
    if deduplicateSmiles:
        predictionInputDF = deduplicateForPrediction(inputDF, smilesColumn)
        print(f"Unique SMILES for prediction: {len(predictionInputDF)}")
    else:
        predictionInputDF = inputDF[[smilesColumn]].dropna().reset_index(drop=True)
        print(f"Total SMILES for prediction: {len(predictionInputDF)}")

    allSmiles = predictionInputDF[smilesColumn].tolist()
    ...
    if resumeFromCheckpoint:
        processedPredictionsDF = loadCheckpoint(paths["checkpointCsvPath"], smilesColumn)
        if processedPredictionsDF is not None:
            processedSmilesSet = set(processedPredictionsDF[smilesColumn].dropna().tolist())
    ...
    if len(smilesRemaining) > 0:
        ...
        for batchNum, (_, smilesBatch) in enumerate(
            ...
            batchPredictionsDF = safePredictBatch(model, smilesBatch, smilesColumn)
        ...
        if saveEveryBatch:
            finalPredictionsDF = compactCheckpoint(paths["checkpointCsvPath"], smilesColumn)
        else:
            raise RuntimeError(
    ...
    mergedDF = mergePredictionsBack(inputDF, finalPredictionsDF, smilesColumn)
