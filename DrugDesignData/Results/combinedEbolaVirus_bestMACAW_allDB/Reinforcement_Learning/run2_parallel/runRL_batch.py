...
This version adds two performance improvements:
1) Batch + parallel oracle calls inside CombinedMoleculeScorer.scoreBatch():
   - ART (predictBatch) and ADMET-tox (scoreBatch) run concurrently via threads.
   - Controlled by: config.performance.oracle_parallelism (default 1).

2) Rollout-only deferred terminal scoring (batch scoring of final SMILES):
   - Controlled by:
       - config.rollout.terminal_scoring_mode: "immediate" | "deferred"
       - config.rollout.final_score_batch_size: batch size for combinedScorer.scoreBatch

Multi-objective + Pareto logic is preserved:
- In deferred mode, we compute scores/rewards after rollout, and apply ParetoArchive.add()
  in the same episode order to keep identical behavior to immediate mode.
"""
...
def buildWeightDF(refScoreDF: pd.DataFrame, endpointMetaDF: pd.DataFrame) -> pd.DataFrame:
    ...
    if weightDF.empty:
        raise ValueError("No endpoint weights could be built. Check reference ADMET columns.")
...
def loadOrBuildReferenceAndWeights(
    ...
    referenceScoredCsv = toxConfig.get("reference_scored_csv")  # may be missing
    referenceInputCsv = toxConfig.get("reference_input_csv")    # used to (re)generate
    referenceSmilesCol = toxConfig.get("reference_smiles_col", "SMILES")
    maxReferenceRows = toxConfig.get("max_reference_rows")

    refScoreDF: Optional[pd.DataFrame] = None

    # ---- 1) Try loading reference_scored_csv if it exists ----
    if referenceScoredCsv:
        refScoredPath = Path(referenceScoredCsv)
        if refScoredPath.exists():
            refScoreDF = normalizeColumns(pd.read_csv(refScoredPath))
            ...
            print(f"[toxicity] Loaded reference scored ADMET from: {refScoredPath}")
        else:
            print(f"[toxicity] reference_scored_csv not found: {refScoredPath}. Will regenerate from reference_input_csv...")

    # ---- 2) If not loaded, generate from reference_input_csv ----
    if refScoreDF is None:
        if not referenceInputCsv:
            raise ValueError(
                "toxicity.reference_scored_csv not found (or not provided) AND toxicity.reference_input_csv is missing. "
                "Provide reference_input_csv so the code can generate reference_admet_scored.csv."
            )

        refInputPath = Path(referenceInputCsv)
        if not refInputPath.exists():
            raise FileNotFoundError(f"toxicity.reference_input_csv not found: {refInputPath}")

        refInputDF = normalizeColumns(pd.read_csv(refInputPath))
        ...
        # Save generated reference scored CSV into outputDir (recommended)
        refOut = outputDir / "reference_admet_scored.csv"
        refScoreDF.to_csv(refOut, index=False)
        print(f"[toxicity] Generated reference ADMET scores and saved to: {refOut}")

    # ---- 3) Load endpoint meta (or use default) ----
    ...
    # ---- 4) Load endpoint weight CSV if exists; else build ----
    weightCsv = toxConfig.get("weight_csv")
    weightDF: Optional[pd.DataFrame] = None

    if weightCsv:
        weightPath = Path(weightCsv)
        if weightPath.exists():
            weightDF = normalizeColumns(pd.read_csv(weightPath))
            print(f"[toxicity] Loaded endpoint weights from: {weightPath}")
        else:
            print(f"[toxicity] weight_csv not found: {weightPath}. Will rebuild endpoint weights...")
    ...
    if weightDF is None:
        ...
        print(f"[toxicity] Saved rebuilt endpoint weights to: {weightOut}")
...
class CombinedMoleculeScorer:
    ...
    Change (performance):
    - scoreBatch runs ART and toxicity scoring concurrently (threads) when oracle_parallelism > 1.
    """
    ...
    def __init__(
        ...
        oracle_parallelism: int = 1,
    ):
        self.artOracle = artOracle
        self.toxicityOracle = toxicityOracle
        self.cache: Dict[str, dict] = {}

        self.oracle_parallelism = max(1, int(oracle_parallelism))
        self._executor = ThreadPoolExecutor(max_workers=2) if self.oracle_parallelism > 1 else None

    def scoreBatch(self, smilesList: List[str]) -> pd.DataFrame:
        canonicalList: List[str] = []
        for smiles in smilesList:
            canonical = canonicalizeSmiles(smiles)
            if canonical is not None:
                canonicalList.append(canonical)
        ...
        newRows: List[dict] = []
        if uncachedList:
            # --- Parallelize ART and toxicity scoring ---
            if self._executor is not None:
                fut_art = self._executor.submit(self.artOracle.predictBatch, uncachedList)
                fut_tox = self._executor.submit(self.toxicityOracle.scoreBatch, uncachedList)
                artDF = fut_art.result()
                toxDF = fut_tox.result()
            else:
...
def computeRewardVector(scoreDict: dict, rewardConfig: dict, routeDepth: int) -> dict:
    ...
    # Uncertainty component is negative so positive weight penalizes it.
    return {
...
def buildEnvClass():
    ...
    class MoleculeRlEnv(gym.Env):
        ...
        def __init__(
            ...
            terminal_scoring_mode: str = "immediate",  # NEW: immediate | deferred
        ):
            super().__init__()
            ...
            self.terminal_scoring_mode = str(terminal_scoring_mode).lower()
        ...
        def _terminalStep(self):
            # Deferred mode: do NOT score here; rollout will batch-score later.
            if self.terminal_scoring_mode == "deferred":
                routeDepth = len(self.routeHistory)
                obs = buildObservation(self.currentSmiles, self.stepIndex, self.seedRow, self.fpBits)
                info = {
                    "finalSmiles": self.currentSmiles,
                    "routeHistory": self.routeHistory,
                    "routeDepth": routeDepth,
                    # keep compatibility-ish
                    "rewardDict": None,
                    "scoreDict": None,
                }
...
def runManualEnvTest(
    ...
    env = EnvClass(
        ...
        preferenceWeights=getDefaultPreferenceWeights(rewardConfig),
        terminal_scoring_mode="immediate",  # keep smoke test immediate
    )
...
def _batch_score_smiles(
    combinedScorer: CombinedMoleculeScorer,
    uniqueSmiles: List[str],
    batchSize: int,
) -> Dict[str, dict]:
    scoreMap: Dict[str, dict] = {}
    for i in range(0, len(uniqueSmiles), batchSize):
        batchSmiles = uniqueSmiles[i:i + batchSize]
        scoredDF = combinedScorer.scoreBatch(batchSmiles)
        if scoredDF.empty:
            continue
        for _, row in scoredDF.iterrows():
            scoreMap[row["SMILES"]] = row.to_dict()
    return scoreMap
...
def rolloutPolicy(
    ...
    terminal_scoring_mode = str(rolloutConfig.get("terminal_scoring_mode", "immediate")).lower()
    final_score_batch_size = int(rolloutConfig.get("final_score_batch_size", 512))
    ...
    env = EnvClass(
        ...
        fpBits=int(rlConfig.get("fingerprint_bits", 2048)),
        paretoArchive=paretoArchive,
        terminal_scoring_mode=terminal_scoring_mode,
    )
    ...
    if terminal_scoring_mode == "deferred":
        episodeRecords = []
        for episodeIndex in range(numEpisodes):
            ...
            if "finalSmiles" in stepInfo:
                episodeRecords.append(
                    {
                        "episodeIndex": episodeIndex,
                        "finalSmiles": stepInfo["finalSmiles"],
                        "routeHistory": stepInfo.get("routeHistory", []),
                        "routeDepth": stepInfo.get("routeDepth", len(stepInfo.get("routeHistory", []))),
                    }
                )

        if not episodeRecords:
            outCsv = outputDir / rolloutConfig.get("output_csv", "rl_generated_candidates.csv")
            pd.DataFrame().to_csv(outCsv, index=False)
            print("Saved generated candidates to:", outCsv)
            return pd.DataFrame()

        uniqueSmiles = list(dict.fromkeys([r["finalSmiles"] for r in episodeRecords]))
        scoreMap = _batch_score_smiles(combinedScorer, uniqueSmiles, final_score_batch_size)

        preferenceWeights = getDefaultPreferenceWeights(rewardConfig)

        # Apply ParetoArchive.add() in SAME order as immediate mode would have
        for rec in episodeRecords:
            smiles = rec["finalSmiles"]
            if smiles not in scoreMap:
                continue
            scoreDict = scoreMap[smiles]

            rewardDict = computeMultiObjectiveReward(
                scoreDict=scoreDict,
                rewardConfig=rewardConfig,
                routeDepth=rec["routeDepth"],
                preferenceWeights=preferenceWeights,
            )

            if paretoArchive is not None:
                paretoBonus = paretoArchive.add(scoreDict)
                rewardDict["totalRewardBeforePareto"] = float(rewardDict["totalReward"])
                rewardDict["paretoBonus"] = float(paretoBonus)
                rewardDict["totalReward"] = float(rewardDict["totalReward"] + paretoBonus)
            else:
            ...
            row = dict(scoreDict)
            row["episodeIndex"] = rec["episodeIndex"]
            row["reward"] = float(rewardDict["totalReward"])
            row["routeLength"] = len(rec.get("routeHistory", []))
        ...
        for episodeIndex in range(numEpisodes):
            ...
            if "scoreDict" in stepInfo and stepInfo["scoreDict"] is not None:
    ...
    for preferenceName, preferenceWeights in preferenceSets.items():
        ...
        env = DummyVecEnv([
            lambda pref=preferenceWeights, archive=paretoArchive: EnvClass(
                ...
                terminal_scoring_mode="immediate",
            )
        ])
...
def rolloutMultiObjectivePolicies(
    ...
    for preferenceName, model in trainedModels.items():
        ...
        if terminal_scoring_mode == "deferred":
            ...
            # Apply ParetoArchive.add() in SAME order as immediate mode
            for rec in episodeRecords:
                smiles = rec["finalSmiles"]
                if smiles not in scoreMap:
                    continue
                ...
                paretoBonus = paretoArchive.add(scoreDict) if paretoArchive is not None else 0.0
                rewardDict["totalRewardBeforePareto"] = float(rewardDict["totalReward"])
                rewardDict["paretoBonus"] = float(paretoBonus)
                rewardDict["totalReward"] = float(rewardDict["totalReward"] + paretoBonus)
                ...
                # Flatten rewardDict exactly like immediate code does
...
def main() -> None:
    ...
    # NEW: oracle_parallelism knob
    performanceCfg = config.get("performance", {})
    oracle_parallelism = int(performanceCfg.get("oracle_parallelism", 1))
