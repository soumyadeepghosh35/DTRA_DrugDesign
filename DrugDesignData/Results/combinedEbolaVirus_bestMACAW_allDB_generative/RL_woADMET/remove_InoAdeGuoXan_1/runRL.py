...
Usage
-----
python runRL_potency_only.py config_potency_only.yaml
...
class DoranetAdapter:
    ...
    def __init__(self, config: dict):
        ...
        # Each generation run writes DORAnet network JSON files into its own
        # per-generation output directory, e.g. RL_outputs_potency_gen_sweep_gen1/doranet_networks.
    ...
    def enumerateActions(self, smiles: str) -> List[DoranetAction]:
        ...
        try:
            ...
            # configured per-generation network directory so gen1/gen2/gen3
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run DORAnet gen sweep + ART potency-only reinforcement-learning workflow."
    )
    ...
    baseOutputDir = Path(config.get("output", {}).get("output_dir", "RL_outputs_potency_gen_sweep"))
    print("Base output directory:", baseOutputDir.resolve())
    ...
    print("DORAnet generation list:", genList)
    ...
    for gen in genList:
        outputDir = buildGenerationOutputDir(baseOutputDir, gen)
        runConfig = makeGenerationConfig(
            config=config,
            gen=gen,
            maxAtoms=maxAtoms,
            outputDir=outputDir,
        ...
        if generatedDF is not None and not generatedDF.empty:
            genGeneratedDF = generatedDF.copy()
            genGeneratedDF["doranetGen"] = int(gen)
            genGeneratedDF["generationOutputDir"] = str(outputDir)
            generatedByGen.append(genGeneratedDF)
            bestPotency = pd.to_numeric(genGeneratedDF["pPotency_prediction"], errors="coerce").max()
            nGenerated = len(genGeneratedDF)
        else:
            bestPotency = np.nan
            nGenerated = 0
        ...
        summaryRows.append(
            {
                ...
                "nGeneratedUnique": int(nGenerated),
                "bestGeneratedPotency": bestPotency,
                "maxAtoms": "|".join(f"{k}:{v}" for k, v in maxAtoms.items()),
            }
        )

    baseOutputParent = baseOutputDir.parent if str(baseOutputDir.parent) != "" else Path(".")
    sweepSummaryDF = pd.DataFrame(summaryRows)
    sweepSummaryPath = baseOutputParent / f"{baseOutputDir.name}_gen_sweep_summary.csv"
    sweepSummaryDF.to_csv(sweepSummaryPath, index=False)
    print("Saved generation sweep summary to:", sweepSummaryPath)

    if generatedByGen:
        combinedGeneratedDF = pd.concat(generatedByGen, ignore_index=True)
        combinedGeneratedDF = combinedGeneratedDF.drop_duplicates(subset=["SMILES"]).reset_index(drop=True)
        sortCols = [col for col in ["pPotency_prediction", "deltaPotency", "reward"] if col in combinedGeneratedDF.columns]
        if sortCols:
            combinedGeneratedDF = combinedGeneratedDF.sort_values(sortCols, ascending=[False] * len(sortCols)).reset_index(drop=True)
        combinedPath = baseOutputParent / f"{baseOutputDir.name}_all_gens_generated_candidates.csv"
        combinedGeneratedDF.to_csv(combinedPath, index=False)
        print("Saved combined candidates across all DORAnet generations to:", combinedPath)

    print(f"Total runtime across DORAnet generation sweep: {time.time() - startTime:.2f} seconds")
