#!/usr/bin/env python3
"""
runRL_potency_frontier_fast_cache.py
...
Usage
-----
python runRL_potency_frontier_fast_cache.py config_gen1.yaml
...
class DoranetAdapter:
    ...
    def __init__(self, config: dict):
        ...
        # When ranked action tables exist, load them directly instead of re-scoring
        # raw DORAnet products with ART in the main PPO process. This preserves
        # the same action selection logic but removes repeated ART-ranking work.
        self.usePrecomputedRankedActions = bool(dconf.get("use_precomputed_ranked_actions", True))
    ...
    def getActionRankedScorePath(self, jobName: str) -> Path:
        return self.actionCacheDir / f"{jobName}_action_ranked_scores.csv"

    def getRankingConfigHash(self) -> str:
        """Hash the pieces that make a ranked action table scientifically valid.

        This prevents stale ranked action files from being reused after changes
        to the action-ranking weights, seed frontier, filters, generation, or
        max-actions settings.
        """
        payload = {
            ...
            "maxRawProducts": self.maxRawProducts,
            "actionSelection": self.actionSelection,
            "actionRankWeights": self.actionRankWeights,
            "diversityConfig": self.diversityConfig,
            "candidateFilterConfig": {k: sorted(v) if isinstance(v, set) else v for k, v in self.candidateFilterConfig.items()},
            "globalBestSeedPotency": round(float(self.globalBestSeedPotency), 8) if np.isfinite(self.globalBestSeedPotency) else None,
            "seedCount": len(self.seedSmilesSet),
        }
        dumped = yaml.safe_dump(payload, sort_keys=True)
        return hashlib.sha1(dumped.encode("utf-8")).hexdigest()

    def loadPrecomputedRankedActions(self, canonical: str, jobName: str) -> Optional[List[str]]:
        """Load already ART-ranked actions if the cache matches the current config."""
        if not self.actionCacheEnabled or not self.usePrecomputedRankedActions:
            return None
        rankPath = self.getActionRankedScorePath(jobName)
        if not rankPath.exists():
            return None
        try:
            rankedDF = pd.read_csv(rankPath)
        except Exception as exc:
            print(f"WARNING: could not read ranked action cache {rankPath}: {exc}")
            return None
        if rankedDF.empty or "productSmiles" not in rankedDF.columns:
            return None
        if "_rankingConfigHash" in rankedDF.columns:
            cachedHash = str(rankedDF["_rankingConfigHash"].dropna().iloc[0]) if rankedDF["_rankingConfigHash"].dropna().shape[0] else ""
            if cachedHash and cachedHash != self.getRankingConfigHash():
                return None
        else:
            # Old ranked files did not record the ranking configuration, so they
            # are unsafe to reuse after code/config changes.
            return None
        sortCols = []
        ascending = []
        if "isSelectedAction" in rankedDF.columns:
            sortCols.append("isSelectedAction")
            ascending.append(False)
        if "actionRankScore" in rankedDF.columns:
            sortCols.append("actionRankScore")
            ascending.append(False)
        if sortCols:
            rankedDF = rankedDF.sort_values(sortCols, ascending=ascending)
        productList = (
            rankedDF["productSmiles"]
            .dropna()
            .astype(str)
        ...
        productList = [p for p in productList if p != canonical]
        return productList[: self.maxActions] if productList else None
    ...
    def enumerateActions(self, smiles: str) -> List[DoranetAction]:
        ...
        # Fast path: if a previous parallel precompute already generated and
        # ART-ranked the action list under the same configuration, use it
        # directly. This avoids repeated MACAW/ART scoring inside PPO.
        rankedProducts = self.loadPrecomputedRankedActions(canonical, jobName)
        if rankedProducts is not None:
            actions = self.buildActionsFromProducts(canonical, jobName, rankedProducts)
        ...
        # Save ranked ART action table. Later PPO/beam-search calls can load this
        # directly without re-running ART ranking if the ranking hash matches.
        if scoredActionDF is not None and not scoredActionDF.empty:
            rankPath = self.getActionRankedScorePath(jobName)
            try:
                scoredActionDF = scoredActionDF.copy()
                scoredActionDF["_rankingConfigHash"] = self.getRankingConfigHash()
                scoredActionDF["isSelectedAction"] = scoredActionDF.get("productSmiles", pd.Series(dtype=str)).isin(set(selectedProducts))
...
_PRECOMPUTE_WORKER_CONTEXT: Dict[str, Any] = {}


def _precomputeRankedActionWorkerInitializer(initPayload: dict) -> None:
    """Initialize one persistent DORAnet+ART action-ranking worker.

    The expensive MACAW transformer and ART model are loaded once per process,
    not once per molecule. Each task then uses the same worker-local adapter and
    scorer to generate raw DORAnet products, filter them, ART-rank them, and
    write both raw and ranked action-cache files.
    """
    workerOmpThreads = str(initPayload.get("workerOmpThreads", "1"))
    ...
    artOracle = ArtPotencyOracle(
        ...
        macawFeatureBuilder=macawFeatureBuilder,
        artOutputDir=config.get("art", {}).get("output_dir"),
        inputFeaturePrefix=config.get("art", {}).get("input_feature_prefix", "MACAW_"),
    )
    combinedScorer = CombinedMoleculeScorer(
        artOracle=artOracle,
        toxicityOracle=None,
    ...
    adapter.seedSmilesSet = set(initPayload.get("seedSmilesSet", []))
    adapter.seedPotencyBySmiles = {str(k): float(v) for k, v in initPayload.get("seedPotencyBySmiles", {}).items()}
    adapter.globalBestSeedPotency = float(initPayload.get("globalBestSeedPotency", np.nan))
    adapter.setActionScorer(combinedScorer)

    _PRECOMPUTE_WORKER_CONTEXT.clear()
    _PRECOMPUTE_WORKER_CONTEXT.update({
        "adapter": adapter,
        "combinedScorer": combinedScorer,
    })


def _precomputeRankedActionWorker(task: dict) -> dict:
    """Precompute one state's raw DORAnet products and ART-ranked actions."""
    startTime = time.time()
    smiles = str(task["smiles"])
    stateIndex = int(task.get("stateIndex", task.get("seedIndex", -1)))
    routeDepth = int(task.get("routeDepth", 0))
    sourcePotency = float(task.get("sourcePotency", task.get("seedPotency", np.nan)))

    try:
        adapter: DoranetAdapter = _PRECOMPUTE_WORKER_CONTEXT["adapter"]
        canonical = canonicalizeSmiles(smiles) or smiles
        actionList = adapter.enumerateActions(canonical)
        jobName = adapter.stableJobName(canonical)
        cachePath = adapter.getActionCachePath(jobName)
        rankPath = adapter.getActionRankedScorePath(jobName)
        topProducts = [action.productSmiles for action in actionList]
        return {
            "stateIndex": stateIndex,
            "routeDepth": routeDepth,
            "stateSmiles": canonical,
            "sourcePotency": sourcePotency,
            "numActions": int(len(actionList)),
            "topProducts": "|".join(topProducts),
            "jobName": jobName,
            "cachePath": str(cachePath),
            "rankPath": str(rankPath),
            "status": "ok",
            "error": "",
...
def precomputeRankedDoranetActionsForStates(
    stateRows: List[dict],
    seedDF: pd.DataFrame,
    config: dict,
    outputDir: Path,
    ...
    """Parallel precompute for arbitrary seed/product states.

    This is the speed-critical replacement for seed-only precompute. It does not
    reduce RL depth or action-space quality: it simply fills the same DORAnet raw
    cache and the same ART-ranked action cache before PPO asks for them.
    """
    outputDir = ensureDir(outputDir)
    if not stateRows:
        emptyDF = pd.DataFrame()
        emptyDF.to_csv(outputDir / summaryName, index=False)
        return emptyDF

    doranetConfig = config.get("doranet", {}) or {}
    requestedWorkers = int(doranetConfig.get("precompute_num_workers", 1))
    maxWorkers = max(1, min(requestedWorkers, len(stateRows)))
    ...
    seedSmilesSet = seedDF["canonicalSmiles"].dropna().astype(str).tolist()
    seedPotencyBySmiles = {
    ...
    initPayload = {
        "config": config,
        "workerOmpThreads": workerOmpThreads,
        "seedSmilesSet": seedSmilesSet,
        "seedPotencyBySmiles": seedPotencyBySmiles,
        "globalBestSeedPotency": globalBestSeedPotency,
    }

    print("\n================ Parallel DORAnet action-cache precompute ================")
    print(f"States to precompute: {len(stateRows)} | workers={maxWorkers} | parallel={useParallel}")
    print(f"OMP threads per worker: {workerOmpThreads}")
    ...
    if useParallel:
        ...
        with futures.ProcessPoolExecutor(
            ...
            initializer=_precomputeRankedActionWorkerInitializer,
            initargs=(initPayload,),
        ) as executor:
            futureList = [executor.submit(_precomputeRankedActionWorker, task) for task in stateRows]
            for doneIdx, futureObj in enumerate(futures.as_completed(futureList), start=1):
                row = futureObj.result()
                rows.append(row)
                print(
                    f"[{doneIdx:04d}/{len(futureList):04d}] "
                    f"depth={row['routeDepth']} stateIndex={row['stateIndex']} "
                    f"status={row['status']} numActions={row['numActions']} "
                    f"time={row['runtimeSeconds']:.2f}s"
                )
    else:
        _precomputeRankedActionWorkerInitializer(initPayload)
        for task in stateRows:
            row = _precomputeRankedActionWorker(task)
            rows.append(row)
            print(
                f"depth={row['routeDepth']} stateIndex={row['stateIndex']} "
                f"status={row['status']} numActions={row['numActions']} "
    ...
    if not summaryDF.empty:
        summaryDF = summaryDF.sort_values(["routeDepth", "stateIndex"]).reset_index(drop=True)
    outPath = outputDir / summaryName
    summaryDF.to_csv(outPath, index=False)
    nOk = int((summaryDF["status"] == "ok").sum()) if not summaryDF.empty and "status" in summaryDF.columns else 0
    nFailed = int((summaryDF["status"] != "ok").sum()) if not summaryDF.empty and "status" in summaryDF.columns else 0
    totalActions = int(pd.to_numeric(summaryDF.get("numActions", pd.Series(dtype=float)), errors="coerce").fillna(0).sum()) if not summaryDF.empty else 0
    print(
        f"Finished ranked action precompute in {time.time() - startTime:.2f}s | "
        f"ok={nOk}, failed={nFailed}, totalActions={totalActions}"
    )
    print("Saved action-cache precompute summary to:", outPath)
    return summaryDF


def _getDepthLimit(value: Any, routeDepth: int, defaultValue: int) -> int:
    if isinstance(value, (list, tuple)):
        if routeDepth < len(value):
            return int(value[routeDepth])
        return int(value[-1]) if value else int(defaultValue)
    return int(value if value is not None else defaultValue)


def _loadRankedProductsForState(doranetAdapter: DoranetAdapter, smiles: str) -> pd.DataFrame:
    canonical = canonicalizeSmiles(smiles)
    if canonical is None:
        return pd.DataFrame()
    jobName = doranetAdapter.stableJobName(canonical)
    rankPath = doranetAdapter.getActionRankedScorePath(jobName)
    if not rankPath.exists():
        return pd.DataFrame()
    try:
        rankedDF = pd.read_csv(rankPath)
    ...
    if rankedDF.empty or "productSmiles" not in rankedDF.columns:
        return pd.DataFrame()
    if "_rankingConfigHash" in rankedDF.columns:
        cleanHash = rankedDF["_rankingConfigHash"].dropna()
        if not cleanHash.empty and str(cleanHash.iloc[0]) != doranetAdapter.getRankingConfigHash():
            return pd.DataFrame()
    rankedDF = rankedDF.copy()
    rankedDF["sourceStateSmiles"] = canonical
    rankedDF["sourceJobName"] = jobName
    return rankedDF


def precomputeFrontierDoranetActions(
    seedDF: pd.DataFrame,
    doranetAdapter: DoranetAdapter,
    combinedScorer: CombinedMoleculeScorer,
    config: dict,
    ...
    """Precompute DORAnet+ART action caches for seed and high-value product states.

    This accelerates PPO without reducing RL depth. PPO can still visit any state;
    uncached states will be generated lazily. The precompute only fills caches for
    the states most likely to be revisited: selected seeds and top ART-ranked
    product states at each route depth.
    """
    doranetConfig = config.get("doranet", {}) or {}
    preconf = doranetConfig.get("frontier_precompute", {}) or {}

    legacyEnabled = bool(doranetConfig.get("precompute_seed_actions", True))
    enabled = bool(preconf.get("enabled", legacyEnabled))
    if not enabled:
        print("DORAnet frontier precompute disabled by config.")
        return

    outputDir = ensureDir(outputDir)
    rlMaxSteps = int((config.get("rl", {}) or {}).get("max_steps", 4))
    maxDepth = int(preconf.get("max_depth", max(0, rlMaxSteps - 1)))
    maxDepth = max(0, min(maxDepth, max(0, rlMaxSteps - 1)))
    maxStatesPerDepth = preconf.get("max_states_per_depth", 512)
    nextStatePoolMultiplier = int(preconf.get("next_state_pool_multiplier", 4))
    failFast = bool(doranetConfig.get("precompute_fail_fast", False))

    seedWorkDF = seedDF.reset_index(drop=True).copy()
    globalBestSeedPotency = float(pd.to_numeric(seedDF["pPotency_prediction"], errors="coerce").max())

    currentStateDF = pd.DataFrame({
        "stateSmiles": seedWorkDF["canonicalSmiles"].astype(str),
        "sourcePotency": pd.to_numeric(seedWorkDF["pPotency_prediction"], errors="coerce"),
        "routeDepth": 0,
        "originSeedSmiles": seedWorkDF["canonicalSmiles"].astype(str),
    })

    seenStates = set(currentStateDF["stateSmiles"].dropna().astype(str).tolist())
    allSummary = []
    allCandidateRows = []

    print("\n================ Frontier DORAnet precompute plan ================")
    print(f"RL max_steps={rlMaxSteps}; precomputing action lists for routeDepth 0..{maxDepth}")
    print(f"This does not reduce RL depth; uncached states are still generated lazily.")

    for routeDepth in range(maxDepth + 1):
        if currentStateDF.empty:
            print(f"No states available for precompute at depth {routeDepth}; stopping.")
            break

        limit = _getDepthLimit(maxStatesPerDepth, routeDepth, 512)
        workDF = currentStateDF.copy()
        if routeDepth > 0:
            sortCols = [col for col in ["frontierDeltaPotency", "pPotency_prediction", "actionRankScore"] if col in workDF.columns]
            if sortCols:
                workDF = workDF.sort_values(sortCols, ascending=[False] * len(sortCols))
            workDF = workDF.head(limit).copy()

        stateRows = []
        for stateIndex, row in workDF.reset_index(drop=True).iterrows():
            stateRows.append({
                "stateIndex": int(stateIndex),
                "routeDepth": int(routeDepth),
                "smiles": str(row["stateSmiles"]),
                "sourcePotency": float(row.get("sourcePotency", np.nan)),
            })

        summaryDF = precomputeRankedDoranetActionsForStates(
            stateRows=stateRows,
            seedDF=seedDF,
            config=config,
            outputDir=outputDir,
            summaryName=f"frontier_action_precompute_depth{routeDepth}.csv",
        )
        if not summaryDF.empty:
            allSummary.append(summaryDF)
            nFailed = int((summaryDF["status"] != "ok").sum()) if "status" in summaryDF.columns else 0
            if nFailed > 0 and failFast:
                raise RuntimeError(f"Frontier precompute failed for {nFailed} states at depth {routeDepth}.")

        if routeDepth >= maxDepth:
            break

        candidateDFList = []
        for _, row in workDF.iterrows():
            rankedDF = _loadRankedProductsForState(doranetAdapter, str(row["stateSmiles"]))
            if rankedDF.empty:
                continue
            if "isSelectedAction" in rankedDF.columns:
                rankedDF = rankedDF.loc[rankedDF["isSelectedAction"] == True].copy()
            rankedDF["originSeedSmiles"] = row.get("originSeedSmiles", row["stateSmiles"])
            rankedDF["sourcePotency"] = row.get("sourcePotency", np.nan)
            rankedDF["routeDepth"] = routeDepth + 1
            candidateDFList.append(rankedDF)

        if not candidateDFList:
            print(f"No ranked products found after depth {routeDepth}; stopping frontier precompute.")
            break

        candidateDF = pd.concat(candidateDFList, ignore_index=True)
        candidateDF["stateSmiles"] = candidateDF["productSmiles"].apply(canonicalizeSmiles)
        candidateDF = candidateDF.dropna(subset=["stateSmiles"]).drop_duplicates("stateSmiles")
        candidateDF = candidateDF.loc[~candidateDF["stateSmiles"].isin(seenStates)].copy()
        if candidateDF.empty:
            print(f"All products at depth {routeDepth + 1} were already seen; stopping.")
            break

        if "pPotency_prediction" not in candidateDF.columns:
            scoredDF = combinedScorer.scoreBatch(candidateDF["stateSmiles"].tolist())
            scoredDF["stateSmiles"] = scoredDF["SMILES"].apply(canonicalizeSmiles)
            candidateDF = candidateDF.merge(
                scoredDF.drop(columns=["SMILES"], errors="ignore"),
                on="stateSmiles",
                how="left",
            )
        candidateDF["frontierDeltaPotency"] = pd.to_numeric(candidateDF["pPotency_prediction"], errors="coerce") - globalBestSeedPotency
        if "actionRankScore" not in candidateDF.columns:
            candidateDF["actionRankScore"] = candidateDF["pPotency_prediction"].rank(pct=True)

        sortCols = [col for col in ["frontierDeltaPotency", "pPotency_prediction", "actionRankScore"] if col in candidateDF.columns]
        candidateDF = candidateDF.sort_values(sortCols, ascending=[False] * len(sortCols)).reset_index(drop=True)

        poolLimit = max(limit, limit * max(1, nextStatePoolMultiplier))
        allCandidateRows.append(candidateDF.head(poolLimit).copy())
        nextLimit = _getDepthLimit(maxStatesPerDepth, routeDepth + 1, limit)
        currentStateDF = candidateDF.head(nextLimit)[["stateSmiles", "sourcePotency", "routeDepth", "originSeedSmiles", "pPotency_prediction", "frontierDeltaPotency", "actionRankScore"]].copy()
        seenStates.update(currentStateDF["stateSmiles"].dropna().astype(str).tolist())
        print(f"Prepared {len(currentStateDF)} high-value product states for depth {routeDepth + 1} precompute.")

    if allSummary:
        summaryPath = outputDir / "frontier_action_precompute_summary_all_depths.csv"
        pd.concat(allSummary, ignore_index=True).to_csv(summaryPath, index=False)
        print("Saved combined frontier action precompute summary to:", summaryPath)
    if allCandidateRows:
        poolPath = outputDir / "frontier_precompute_candidate_pool.csv"
        pd.concat(allCandidateRows, ignore_index=True).drop_duplicates("stateSmiles").to_csv(poolPath, index=False)
        print("Saved frontier precompute candidate pool to:", poolPath)
...
def runSingleGenerationWorkflow(
    ...
    precomputeFrontierDoranetActions(
        seedDF=seedDF,
        doranetAdapter=doranetAdapter,
        combinedScorer=combinedScorer,
        config=config,
