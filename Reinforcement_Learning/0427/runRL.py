...
YAML-driven reinforcement-learning workflow for reaction-constrained molecular
optimization with DORAnet, MACAW, ART, and ADMET-AI.
...
def computeRlReward(scoreDict: dict, rewardConfig: dict, routeDepth: int) -> dict:
    weights = rewardConfig.get("weights", {})
    ...
    totalReward = (
        float(weights.get("potency", 0.55)) * potencyReward
        + float(weights.get("toxicity", 0.30)) * toxicityReward
        + float(weights.get("qed", 0.10)) * qedReward
        + float(weights.get("route", 0.05)) * routeReward
        - float(weights.get("uncertainty_penalty", 0.20)) * uncertaintyPenalty
    )

    return {
        "potencyReward": potencyReward,
        "toxicityReward": toxicityReward,
        "qedReward": qedReward,
...
def buildEnvClass():
    ...
    class MoleculeRlEnv(gym.Env):
        ...
        def _terminalStep(self):
            ...
            rewardDict = computeRlReward(scoreDict, self.rewardConfig, len(self.routeHistory))
